...
We see that we have actually loaded a number of packages (which could also be loaded individually): .hi-slate[ggplot2], .hi-slate[tibble], .hi-slate[dplyr], etc.
...
We'll use several of these additional packages during the remainder of this course: .hi-slate[haven] for loading STATA files, .hi-slate[lubridate] for working with dates, .hi-slate[rvest] package for webscraping.
...
These are the workhorse packages for cleaning and wrangling data that you will likely make the most use of (alongside .hi-slate[ggplot2], which we'll cover more in depth later on)
...
german_cars_grp <- group_by(german_cars, manufacturer)
summarise(german_cars_grp, 
          hwy_mean = mean(hwy),
          hwy_max = max(hwy), 
          "Minimum Highway MPG" = min(hwy)
          )

rm(german_cars, german_cars_grp)
```
...
```{r, eval = F}
summarise(group_by(filter(mpg, manufacturer %in% c("audi", "volkswagen")), manufacturer, model),
          hwy_mean = mean(hwy))
```
...
# Pipes: Alternate Argument

By default, the pipe sends what's upstream into the .hi-medgrn[first argument] of the downstream function.

If you want to pass it to a .hi-blue[different argument], place `.` in the target argument.

--

.pull-left[
.hi-medgrn[Default:] $\ln(5)$
```{r}
5 %>%
  log()
```
...
.pull-right[
.hi-blue[Different Argument:] $\log_{5}(10)$
```{r}
5 %>%
  log(10, .)
```
...
# The base R pipe: different arguments

One .hi-red[difference] in base R pipe syntax: piping into .hi-blue[different arguments:]

  * Use `_` (underscore) in place of `.` 
  * Must .hi-blue[name the argument] you're piping into
...
- 1.0.0 has been around for a while now (currently on 1.2.1), but if you have an old old version of dplyr, these functions won't be available
...
`filter(df, condition1, ...)` only keeps rows that match the specified condition(s)
...
.pull-left[
.hi-blue[Character variable:] (in)equality, regular expressions
```{r, eval = F}
starwars |>
...
.pull-right[
.hi-medgrn[Numeric variable:] (in)equality, comparisons
```{r, eval = F}
starwars %>%
filter(
  between(mass, 75, 150) 
...
`filter()` only keeps rows that match the specified condition(s)
...
# 1) dplyr::filter
We can chain multiple filter commands with the pipe (`%>%, |>`), or supply them as separate arguments within a single filter command.
```{r}
starwars |>
filter(species == "Human", mass > 75, mass < 85, 
       height %in% 170:180) 
```
...
```{r filter_stringr}
starwars <- starwars %>% 
  mutate(skywalker = str_detect(name, "Skywalker"),
         not_skywalker = !str_detect(name, "Skywalker"))

sky_check <- select(starwars, name, skywalker) %>%
  arrange(desc(skywalker), name)
```
---
...
Or use the convenient `drop_na()` verb:
  ...
  * Drops `NA` on the named variables
  * If none specified, drops `NA` in .hi-medgrn[any variable]
...
We can also .hi-medgrn[nested sort] by including .hi-medgrn[multiple variables]
  - Sort on first variable, then ties on the next, etc.
```{r arrange_nest}
sw_ar_1 <- starwars %>% 
  arrange(desc(birth_year), name)
```
...
`select()` lets you .hi-pink[subset columns by name/condition]
  * Places variable name arguments in specified order
  * Can also use `first:last` syntax for currently consecutive columns
...
`select()` also works well with [helper functions](https://dplyr.tidyverse.org/reference/select.html)
...
`select()` also works well with [selection helper functions](https://dplyr.tidyverse.org/reference/select.html)
...
You can also .hi-medgrn[rename] some (or all) of your selected variables while changing places.
```{r select2}
starwars %>%
  select(height_cm = height, everything()) %>%
  head()
```
...
Note: you can .hi-purple[rename columns] without subsetting with `rename`.
...
Some other [selection helper functions](https://dplyr.tidyverse.org/reference/select.html):


| Operator/Function | Role |
|---| ---|
| `!` | Take complement of variable set |
| `&` | intersection of two sets |
| `last_col()` | furthest right column |
...
| `group_cols()` | variables defining group structure |
...
starwars %>% 
  ...
  mutate(tall2 = if_else(height > 180, "Tall", "Short")) ## Same effect, but can choose labels
...
Alternatively, we can provide an "anonymous" .hi-medgrn[lambda function] using syntax from the .hi-slate[purrr] package:
  - `\(x)` to indicate we're building an anonymous lambda function 
...
starwars %>% select(name:eye_color) %>% 
  mutate(across(c(height, mass),   \(x) x / 100)) %>% #<< 
  head(5)
```
...
Several other case-specific `slice` versions exist:

| Function | Role |
| -------- | ---- |
| `slice_head/tail(df, n, prop, by)` | Grab the first `n`/`prop` top or bottom rows |
| `slice_min/max(df, order_by, ...)` | Grab the `n`/`prop` min or max value rows |
| `slice_sample(df, replace = F, ...)` | Grab random sample of rows |
...
.pull-right[
`distinct()`: first unique observations.

```{r dist}
starwars <- mutate(starwars,
                   hair_color = if_else(name == "Luke Skywalker", "blonde", hair_color))

starwars %>% distinct(species)

# average height by hair color
starwars %>%
  group_by(hair_color) %>%
...
By default, `distinct()` .hi-medgrn[drops all other variables].

If we're in a context where it makes sense to keep them (i.e. removing a few duplicated rows in a panel dataset), set `.keep_all = T`:

```{r dist2}
starwars %>% distinct(species, .keep_all = T) %>% dim()
starwars %>% distinct(species, .keep_all = T) %>% head()
```
...
The `weather` dataset contains info on the prevailing NYC weather at every minute
...
Two main ways to use `by` when merging on multiple columns:
  ...
  .center[`by = c("xvar1" = "yvar1", "xvar2" = "yvar2", ...)`]
...
A somewhat different mutating join is the .hi-blue[cross join].

`cross_join(df1, df2)`  matches each row in `df1` with each row in `df2` 

  * Results in a `nrow(df1) * nrow(df2)` row dataframe
  * Ignores the data in the columns
    * i.e. doesn't have a `by` argument
  * Useful for expanding an object with all possible combinations of values
  
---
# join_by()

Another helpful tool for conducting .hi-medgrn[conditional joins] is `join_by()`

  * Equality (`==`) or inequality (`<, >, <=, >= !=`)
  * Rolling (`closest()`)
  * Overlap (`between(), within(), overlaps()`)
  * Place in the `by` argument

---
# join_by()

Example: add to weather the info for the first flight that occurred each hour into the weather data
```{r}
# get all flights on 9/10, convert scheduled departure time to combined numeric
flights_9_10 <- filter(flights, month == 9 & day == 10) %>%
  mutate(dep_num = hour + minute/60) %>%
  select(-hour)


weather_close <-  left_join(weather_sub, flights_9_10,
            by = join_by(year, month, day, # variables to join on #<<
                         closest(hour <= dep_num)         )) %>% # condition to join on #<<
  arrange(tailnum) # bring the matches to the top
```

---
# join_by()

Note that we get .hi-red[duplicated matches]. 
...
.hi-slate[A:] weather isn't unique on the matching variables!

```{r}
# group on the hour of each day
group_by(weather_sub, year, month, day, hour) %>%
  # get only the 2nd or higher entry by hour of each day
  filter(row_number() > 1) %>% 
...
Get the first weather observation each hour, then rejoin

```{r}
weather_dist_9_10 <- distinct(weather_sub, year, month, day, hour, .keep_all = T) %>%
  filter(month == 9 & day == 10) 

weather_close <-   
  left_join(weather_dist_9_10, flights_9_10,
            by = join_by(year, month, day, # variables to join on
                         closest(hour <= dep_num)         )) # condition to join on
```
...
Dang, we *still* have too many rows

  * `weather_dist_9_10` has `r nrow(weather_dist_9_10)` rows
  * `weather_close` has `r nrow(weather_close)`!
...
.hi-slate[A:] multiple flights share the same scheduled departure time!

```{r}
group_by(flights_9_10, dep_num) %>%
   filter(row_number() > 1) %>% 
  # how many rows are duplicates?
  nrow()
```
.hi-slate[Important lesson:] **always** 

  1. Understand/check for uniqueness *before* merging
  2. Make sure the rows of the returned dataframe match your expectations
  3. If not, **investigate!**
...
tidyr has several `separate_direction_method` functions that make it easy to separate values in a column into multiple columns/rows, where 
