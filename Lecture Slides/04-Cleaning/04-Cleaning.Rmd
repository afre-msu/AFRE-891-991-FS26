...
```{r, warning = F}
pacman::p_load(haven, nycflights13, sjlabelled, stringr, tidyverse)
```
---
...
* This is an example of a full or .hi-slate[absolute path].
  ...
  * With .R scripts, goes to the default working directory 
    * Change in Tools > Global Options
  * With .Rmd files, defaults to the .hi-medgrn[folder containing the Rmd file]
...
* You have a file saved at `"C:/Github/AFRE-891-991-FS25/assignment-1/assignment-1.Rmd"`
  * This is its .hi-medgrn[absolute path]
...
.hi-blue[Option 1: use relative paths within GitHub repositories]
  * In the main folder (root directory), place 
    ...
    * A **master script/R Markdown file** that does everything itself or lists and runs all other scripts<sup>1</sup>
  * Use paths .hi-blue[relative] to included folder structure specific to each type of input/output, a la...
 
 .footnote[<sup>1</sup> Run another script/file from your current one with `source(script.R)`]
---
...
In .hi-slate[R], navigate .hi-medgrn[up one folder] with `../`

.pull-left[
For example, our current working directory is the `04-Cleaning` folder:
...
--
.pull-right[
And to see the files one folder up (in `Lecture Slides`):
```{r}
list.files("../")
```
]
...
.pull-right[
And one more folder up (in `AFRE-891-991-FS26`):
```{r}
list.files("../../")
```
]
...
The tidyverse package .hi-slate[readr] provides lots of options to read data into R. Read in the college COVID data using `read_csv()` and the .hi-purple[relative filepath]:
